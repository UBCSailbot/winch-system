 ...
 *
 *
 *  Purpose:    removes current active command from the list and increments cmd_index
 *              to the next available command. NOP if no active commands
 *
 *  Params:     none
 *
 *  Return:     none
...
static t_cmd * get_current_command(void) {
    if (num_active_cmd <= 0) return idle_cmd;
